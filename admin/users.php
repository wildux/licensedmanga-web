...
//if not superuser
if(!$user->is_superuser()){ header('Location: /index.php'); }
...
if(isset($_GET['deluser'])){ 
	...
	if($_GET['deluser'] !='1'){
		...
		$stmt = $db->prepare('DELETE FROM blog_members WHERE id = :id') ;
...
<body>
	...
	<?php
		try {
			...
			$stmt = $db->query('SELECT id, username, email FROM blog_members ORDER BY username');
